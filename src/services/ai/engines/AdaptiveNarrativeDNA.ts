...
export class AdaptiveNarrativeDNA {
   ...
   * Now accepts actual measured latency instead of hard-coded 5000ms constant
   */
  evolveNarrative(playerChoice: string, responseTimeMs: number, worldState: WorldState): void {
    const selectionPressure = this.calculateSelectionPressure(playerChoice, responseTimeMs, worldState);
  ...
  /**
   * FIXED: Critical Issue #6 - Hard-coded Response Latency
   * Now uses actual responseTimeMs parameter instead of hard-coded 5000ms
   */
  private calculateSelectionPressure(choice: string, responseTimeMs: number, worldState: WorldState): number {
    let pressure = 0.0;

    // Quick decisions indicate engagement (positive pressure)
    // Using actual response time instead of hard-coded values
    if (responseTimeMs < 5000) pressure += 0.2;
    else if (responseTimeMs > 15000) pressure -= 0.1;
