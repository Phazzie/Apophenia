...
/**
 * Unified text generation with enhanced error handling and retry logic
 * Routes to the selected AI model with intelligent fallback strategies
 */
export async function generateWithSelectedModel(
  ...
  const maxRetries = 1; // Conservative retry for this function
  let lastError: Error | null = null;
  
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      if (attempt > 0) {
        console.log(`Retry attempt ${attempt} for ${selectedModel.name}`);
      ...
      console.warn(`${selectedModel.name} attempt ${attempt + 1} failed:`, lastError.message);
  ...
  // Final fallback with enhanced error context
  console.error(`${selectedModel.name} failed after ${maxRetries + 1} attempts, falling back to Gemini:`, lastError);
  return await generateWithGemini(systemInstruction, prompt, useCase);
}
