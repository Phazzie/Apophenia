 ...
 * with natural language prompts and contextual understanding.
 */
...
export class GeminiImageService extends BaseImageService {
  ...
  async isAvailable(forceCheck: boolean = false): Promise<boolean> {
    ...
    try {
      // Attempt to get the model - throws if model doesn't exist or key is invalid
      const model = this.genAI.getGenerativeModel({ model: this.model });
      // If we got here, the model exists and key is valid
      return !!model;
    } catch (err) {
  ...
  async generate(prompt: string): Promise<ImageResult> {
    ...
    try {
      ...
      const imageModel = this.genAI.getGenerativeModel({
        model: this.model,
        // No generationConfig needed - gemini-2.5-flash-image returns images by default
        // Image data will be in response.candidates[0].content.parts[0].inlineData
      });
      ...
      // Race between image generation and timeout, clear timeout when done
