 ...
 * Includes timeout protection for all AI calls
 */
export class AdaptiveHorrorEngine {
  ...
  async analyzePlayerChoice(choice: string, context: string): Promise<void> {
    // Mock feature check
    const REVOLUTIONARY_FEATURES = { ADAPTIVE_HORROR: { enabled: true } };
    ...
    try {
      // Uses unifiedAIService with timeout protection and fallback cascade
      ...
      if (commands[0]?.type === 'displayText') {
        const triggers = commands[0].payload.content.split(',').map((t: string) => t.trim());
  ...
  async generatePersonalizedHorror(basePrompt: string): Promise<string> {
    ...
    if (personalizedElements) {
      ...
      try {
        // Uses unifiedAIService with timeout protection
        const commands = await generateWithSelectedModel(
          systemInstruction,
          prompt,
          'story'
